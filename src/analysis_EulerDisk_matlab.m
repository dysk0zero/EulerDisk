...
filename = 'recording_04.wav';
[x,fs]=audioread(filename);
x(3e6)=0;                 % signal extended to 30 seconds (to avoid border effects around 26.2 sec)
...
% Observaciones:
%   Señal con un proceso que acaba en el segundo 26.05 o 26.06
%   A medida que nos acercamos al tiempo límite, el proceso se repite cada
%   vez más rápido

%% Analisis espectral
[Px,f]=pwelch(x,hamming(1024*4),512*4,2048*4,fs);
figure(2);
...
% Observaciones:
%   Señal filtrada paso-baja 20-22 kHz (filtrada probablemente por el propio micrófono) 
%   Picos en torno a 2, 5, 8.3, 10.9, 17.4, 20.08 kHz. Hay otros picos,
%   menos estrechos

%% Espectrograma
[X,f,ts] = spectrogram(x,hamming(2048),1024,4096,fs);
figure(3);
imagesc(10*log10(abs(X)),'XData',ts,'YData',f/1000); 
set(gca,'YDir','normal')
colormap('jet')
ylim([0 20]);
...
%% Sugerencia: análisis de señal paso-banda entre 2500 y 4500 (evitando
% armonicos)
...
y=filter(B,1,x);
yo=filter(Bo,1,x);
a0 = abs(y+1j*yo);  % ENVOLVENTE DE LA SEÑAL EN LA BANDA SELECCIONADA
figure(4);
...
[A0,f,ts] = spectrogram(a2,hamming(512),500,4096,fs/100);
...
figure(6);
imagesc(10*log10(abs(A0)+0.01),'XData',ts,'YData',f); 
set(gca,'YDir','normal')
colormap('jet')
ylim([0 200]);
...
%% Estimacion de la frecuencia del tintineo (máximo del espectrograma)
PSD_A0=(abs(A0)).^2;
[Pm,idxm]=max(PSD_A0);
fmax=f(idxm);
fmax_smooth=fmax;
...
if 0
% op=audioplayer(y,fs);       % x: señal original; y: señal en la banda seleccionada
% play(op);

a3=resample(a2,10,1);
a3=a3/max(abs(a3))*0.8;
op=audioplayer(a3,fs/10);  % a2: envolvente submuestreada y filtrada HP
play(op)
...
% buscamos el valor óptimo de t0
t0_opt=-10000;
Error_optimo=1e50;
